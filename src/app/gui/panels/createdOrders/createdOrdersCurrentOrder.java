...
public final class createdOrdersCurrentOrder extends javax.swing.JPanel {

    private static final String[] FOOD_CATEGORIES = Database.PredefinedData.getFoodCategories();
    private final orderNewBase PARENT;

    public createdOrdersCurrentOrder(orderNewBase PARENT) {
        initComponents();
        this.PARENT = PARENT;
        init();
        loadFoodNames();
    ...
    private void loadFoodNames() {
        ...
        for (int i = 0; i < FOOD_CATEGORIES.length; i++) {
            JButton buttonInstance = getCustomizedButton("      " + FOOD_CATEGORIES[i] + "      ", i, bounds);
            foodItemsViewPanel.add(buttonInstance);
            bounds += buttonInstance.getPreferredSize().getWidth() + 50;
            foodListButtonGroup.add(buttonInstance);
        }

    }

    private JButton getCustomizedButton(String ButtonName, int current_index, int bounds) {
        JButton buttonInstance = new JButton(ButtonName);
        buttonInstance.setHorizontalAlignment(JButton.CENTER);
        buttonInstance.setBounds(bounds, 0, buttonInstance.getPreferredSize().width, 40);
        ...
        buttonInstance.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                for (Enumeration<AbstractButton> buttons = foodListButtonGroup.getElements(); buttons.hasMoreElements();) {
